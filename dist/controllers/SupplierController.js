...
export class SupplierController {
    async getAllSuppliersController(c) {
        try {
    ...
    async getSupplierByIdController(c) {
        try {
    ...
    async addSupplierController(c) {
        try {
            const { name, address, contact, service, overallRating } = await c.req.json();
            const newSupplier = await addSupplier(name, address, contact, service, overallRating);
    ...
    async updateSupplierController(c) {
        try {
            const id = Number(c.req.param('id'));
            const { name, address, contact, service, overallRating, } = await c.req.json();
            const updatedSupplier = await updateSupplier(id, name, address, contact, service, overallRating);
            if (!updatedSupplier) {
                return c.json({ message: 'Supplier not found' }, 404);
            }
    ...
    async deleteSupplierController(c) {
        try {
            const id = Number(c.req.param('id'));
