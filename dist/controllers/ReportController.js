...
export class ReportController {
    async getAllReportsController(c) {
        try {
    ...
    async getReportByIdController(c) {
        try {
    ...
    async addReportController(c) {
        try {
            const { description, type, date, idVehicle, idSupplier, idDriver, supplierRating } = await c.req.json();
            const newReport = await addReport(description, type, date, idVehicle, idSupplier, idDriver, supplierRating);
    ...
    async updateReportController(c) {
        try {
            const id = Number(c.req.param('id'));
            const { description, type, date, idVehicle, idSupplier, idDriver, supplierRating } = await c.req.json();
            const updatedReport = await updateReport(id, description, type, date, idVehicle, idSupplier, idDriver, supplierRating);
            if (!updatedReport) {
                return c.json({ message: 'Report not found' }, 404);
            }
    ...
    async deleteReportController(c) {
        try {
            const id = Number(c.req.param('id'));
