...
export class MaintenanceController {
    async getAllMaintenancesController(c) {
        try {
    ...
    async getMaintenanceByIdController(c) {
        try {
    ...
    async addMaintenanceController(c) {
        try {
            const { maintenanceCost, maintenanceDescription, maintenanceDate, maintenanceType, vehicleId, createdAt } = await c.req.json();
            const newMaintenance = await addMaintenance(maintenanceCost, maintenanceDescription, maintenanceDate, maintenanceType, vehicleId, createdAt);
    ...
    async updateMaintenanceController(c) {
        try {
            const id = Number(c.req.param('id'));
            const { maintenanceCost, maintenanceDescription, maintenanceDate, maintenanceType, vehicleId, createdAt } = await c.req.json();
            const updatedMaintenance = await updateMaintenance(id, maintenanceCost, maintenanceDescription, maintenanceDate, maintenanceType, vehicleId, createdAt);
    ...
    async deleteMaintenanceController(c) {
        try {
            const id = Number(c.req.param('id'));
