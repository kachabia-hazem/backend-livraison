...
export class SectorController {
    async getAllSectorsController(c) {
        try {
    ...
    async getSectorByIdController(c) {
        try {
    ...
    async addSectorController(c) {
        try {
            const { name, city, delegation, idVehicle, createdAt, } = await c.req.json();
            const newSector = await addSector(name, city, delegation, idVehicle, createdAt);
    ...
    async updateSectorController(c) {
        try {
            const id = Number(c.req.param('id'));
            const { name, city, delegation, idVehicle, updatedAt, } = await c.req.json();
            const updatedSector = await updateSector(id, name, city, delegation, idVehicle, updatedAt);
            if (!updatedSector) {
                return c.json({ message: 'Sector not found' }, 404);
            }
    ...
    async deleteSectorController(c) {
        try {
            const id = Number(c.req.param('id'));
